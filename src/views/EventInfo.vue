...
export default {
  ...
  computed:{
    singleEvent(){
      return this.meetUpEvents.filter((evnt) =>{ 
        if( evnt.id == this.$route.params.id){
          this.returnedEvent = evnt;
        }
      });
    }
  }
...
@import url('https://fonts.googleapis.com/css?family=Muli&display=swap');
  .section-event-Info{
      background-image: linear-gradient(45deg, #cfcfd8, #ecebf8);
    ...
    .course {
      ...
      .course-preview {
          ...
          .course-preview .course-perview-p {
              color: #fff;
